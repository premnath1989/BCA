    ...
    if (selectedIndex != _selectedIndex && selectedIndex < [self.viewControllers count])
    {
        ...
        if (selectedIndex == MenuOption.HomeIndex) {
            ...
            if (!zzz.SICompleted) {
                ...
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@" " message:@"Are you sure you want to discard all the changes?" delegate:self cancelButtonTitle:@"OK" otherButtonTitles:@"Cancel",nil];
                [alert setTag:2001];
                [alert show];
                alert = Nil;
            }
...
//        [self updateTabBar];
    ...
    else if (alertView.tag == 3001 && buttonIndex == 0)
    {
        ...
        zzz.SICompleted = YES;
        zzz.ExistPayor = YES;
//        NSLog(@"TESTT  click: %d   selected: %d  count:%d", clickIndex, _selectedIndex, [[self viewControllers] count]);
        
        if (_selectedIndex == 3) {
            // clean the Trad_LAPayor table of empty entries
//            [self cleanDatabase];
        }
        UIViewController *selectedViewController = [self.viewControllers objectAtIndex:clickIndex];
        [self addChildViewController:selectedViewController];
...
//        NSString *querySQL = @"DELETE FROM Trad_LAPayor WHERE SINo='(null)'";
...
//        }
//        
//        querySQL = @"DELETE FROM Trad_LAPayor WHERE SINo IS NULL";
//        if(sqlite3_prepare_v2(contactDB, [querySQL UTF8String], -1, &statement, NULL) == SQLITE_OK) {
//            
//            if (sqlite3_step(statement) == SQLITE_DONE) {
//                
...
//        }
//        
//        querySQL = @"DELETE FROM Clt_Profile WHERE CustCode='null'";
//        if(sqlite3_prepare_v2(contactDB, [querySQL UTF8String], -1, &statement, NULL) == SQLITE_OK) {
//            
//            if (sqlite3_step(statement) == SQLITE_DONE) {
//                
...
{
    /* Commented by Edwin 25-03-2014 : never been called.
    AppDelegate *delegate= (AppDelegate*)[[UIApplication sharedApplication] delegate ];
    
	if(delegate.bpMsgPrompt)
    {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@" " message:delegate.bpMsgPrompt delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil,nil];
